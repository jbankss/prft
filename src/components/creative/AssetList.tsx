...
export function AssetList({
  ...
  const getAssetUrl = (asset: Asset) => {
    const {
      data
    } = supabase.storage.from(asset.bucket).getPublicUrl(asset.file_path);
  ...
  return <>
      <div className="space-y-3">
        ...
        return <div key={asset.id} className={`flex items-center gap-4 p-4 rounded-xl border transition-all cursor-pointer group hover-lift ${isSelected ? 'bg-primary/10 border-primary' : 'bg-card/50 border-border hover:bg-card'}`} onClick={e => {
          if ((e.target as HTMLElement).closest('.checkbox-wrapper')) return;
          setSelectedAsset(asset);
        }}>
              {/* Checkbox */}
              {onToggleSelect && <div className="checkbox-wrapper" onClick={e => e.stopPropagation()}>
                  <Checkbox checked={isSelected} onCheckedChange={() => onToggleSelect(asset.id)} className="h-5 w-5" />
                </div>}

              {/* Thumbnail */}
              <div className="w-14 h-14 rounded-lg overflow-hidden flex-shrink-0 bg-muted/50">
                {isImage ? <img src={getAssetUrl(asset)} alt={asset.title || asset.file_name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center">
